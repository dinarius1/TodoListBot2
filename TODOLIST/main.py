        ...
        while True:
            ...
            for i, task in enumerate(self.todos):
                if task.deadline < now:
                    self.delete_task(i)
            await asyncio.sleep(60)  # Проверять сроки выполнения каждые 60 секунд
...
async def start_checking_deadlines():
    while True:
        await todo_list.check_deadlines()


# Начать проверку сроков выполнения
...
async def add_handler(message: types.Message):
    ...
    if task and deadline:
        try:
            ...
            await message.reply(f"Задача '{task}' добавлена в список.", reply_markup=main_keyboard())
        except:
            await message.reply(
                "Неправильный формат дедлайна или вы неправильно ввели задачу. Используйте 'YYYY-MM-DD HH:MM'.")
    else:
        await message.reply("Пожалуйста, введите задачу и дедлайн в формате 'YYYY-MM-DD HH:MM' после команды /add.")
...
async def delete_handler(message: types.Message):
    ...
    if 0 <= index < len(tasks):
        ...
        await message.reply("Неправильный номер задачи.", reply_markup=main_keyboard())
...
async def update_handler(message: types.Message):
    try:
        ...
        if 0 <= index < len(tasks):
            ...
            await message.reply(f"Задача с номером {index + 1} обновлена.", reply_markup=main_keyboard())
        else:
            await message.reply("Неправильный номер задачи.", reply_markup=main_keyboard())
    except ValueError:
        await message.reply("Пожалуйста, введите номер и обновленную задачу после команды /update.")
...
async def inline_button_handler(callback_query: types.CallbackQuery):
    ...
    if command == 'add':
        ...
        "Введите задачу и дедлайн в формате 'YYYY-MM-DD HH:MM' после команды /add.")
    elif command == 'list':
        tasks = todo_list.get_tasks()
        if tasks:
            response = "Список задач:\n"
        ...
        await bot.send_message(callback_query.from_user.id,
                               "Введите номер и обновленную задачу после команды /update.")
...
if __name__ == '__main__':
    ...
    loop.create_task(start_checking_deadlines())  # Запуск фоновой задачи проверки дедлайнов
